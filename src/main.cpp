    ...
    while (std::getline(top, teamChunk, '|')) {
        ...
        while (ts >> tok) {
            if (tok.size() == 1 && !std::isdigit(static_cast<unsigned char>(tok[0]))) {
                char sym = tok[0];
                if (!rosterSet.count(sym)) throw std::runtime_error("Unknown symbol: " + tok);
                team.push_back(sym);
        ...
        if (!team.empty()) teams.push_back(std::move(team));
    }
    if (teams.empty()) throw std::runtime_error("No teams parsed");
...
int main() {
    ...
    if (yn == 'y' || yn == 'Y') {
        ...
        std::cout << "Enter teams as symbols or indices (space-separated), teams separated by '|'\n";
        std::cout << "Examples:\n";
